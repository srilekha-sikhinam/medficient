...
tt2 = TransformedTargetRegressor(regressor=GradientBoostingRegressor(learning_rate = 0.1, max_depth = 9, n_estimators = 100, subsample = 0.5), func=np.log, inverse_func=np.exp)
tt2.fit(X_test_all_features, y_test_all_features)

filename = 'models/finalized_length_of_stay_gradient_boosted_regressor_model'
